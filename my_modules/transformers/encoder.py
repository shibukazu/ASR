...
import copy
from modules.transformers.positional_encoder import PositionalEncoder
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, in_size=256, in_hidden_size=2048, nhead=4, dropout=0.1, norm_first=True):
        ...
        self.multi_head_self_attn = nn.MultiheadAttention(embed_dim=in_size, num_heads=nhead, dropout=dropout, batch_first=True)
...
class TransformerEncoder(nn.Module):
    def __init__(self, in_size=256, nlayer=12, nhead=4, in_hidden_size=2048, dropout=0.1, norm_first=True) -> None:
        ...
        self.nlayer = nlayer
        self.positional_encoding = PositionalEncoder(in_size=in_size)

        transformer_encoder_layer = TransformerEncoderLayer(in_size=in_size, in_hidden_size=in_hidden_size, nhead=nhead, dropout=dropout, norm_first=norm_first)
