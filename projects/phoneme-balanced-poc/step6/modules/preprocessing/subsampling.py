...
class Conv2DSubSampling(nn.Module):
    def __init__(self, in_size, out_size, kernel_size1=3, stride1=2, kernel_size2=3, stride2=1):
        # args:
        #   in_size: 元の特徴量次元数
        ...
        # 畳み込み演算後の特徴量軸のサイズ
        conv_out_size = self.output_size(
            self.output_size(in_size, self.kernel_size1, self.stride1), self.kernel_size2, self.stride2)
    ...
    def forward(self, x, x_lengths):
        ...
        subsampled_x = x
        # 畳み込み演算後の時系列軸のサイズ
        subsampled_x_lengths = self.output_size(
            self.output_size(x_lengths, self.kernel_size1, self.stride1), self.kernel_size2, self.stride2)
