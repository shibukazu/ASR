...
class CausalLayerNormalization(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        cum_mean = cum_x.sum(dim=1).unsqueeze(1).repeat(1, x.shape[1], 1) / cum_num_element
        cum_var = ((cum_x - cum_mean) ** 2).sum(dim=1).unsqueeze(1).repeat(1, x.shape[1], 1) / cum_num_element
        cum_std = torch.sqrt(cum_var + self.eps)
        cum_std = cum_std + self.eps
        normalized_x = (x - cum_mean) / cum_std
        if self.affine:
