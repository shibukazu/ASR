import torch
from modules.conformer.conformer import (
    CausalConformerAdapterBlock,
    CausalConformerAfterLNAdapterBlock,
...
class CausalConformerAdapterEncoder(torch.nn.Module):
    def __init__(
        self,
        input_size,
        subsampled_input_size,
        ...
        self.conformer_blocks = torch.nn.ModuleList(
            [
                ...
                for _ in range(num_conformer_blocks - num_adapter_blocks)
            ]
        )
    ...
    def forward(self, padded_input, input_lengths):
        ...
        for adapter_block in self.adapter_blocks:
            output = adapter_block(output, subsampled_input_lengths)
        for conformer_block in self.conformer_blocks:
            output = conformer_block(output, subsampled_input_lengths)
...
class CausalConformerAfterLNAdapterEncoder(torch.nn.Module):
    def __init__(
        self,
        input_size,
        subsampled_input_size,
