...
def main(cfg: DictConfig):
    ...
    with mlflow.start_run(experiment_id=experiment_id) as mlflow_run:
        ...
        if cfg.selection.type == "random":
            ...
            assert train_dataset[11][5] == "Don't ask me to carry an oily rag like that."
            assert train_dataset[0][5] == "She had your dark suit in greasy wash water all year."
            with open("timit_quantized_indices_memory.pkl", "rb") as f:
        ...
        elif cfg.selection.type == "kl_divergence":
            logger.info("Using KL divergence sampler.")
            # quantizeと同一のデータセットが利用されていることの確認
            assert train_dataset[11][5] == "Don't ask me to carry an oily rag like that."
            assert train_dataset[0][5] == "She had your dark suit in greasy wash water all year."
            with open("timit_quantized_indices_memory.pkl", "rb") as f:
                quantized_indices_memory = pickle.load(f)
            assert len(quantized_indices_memory) == len(train_dataset)
            kl_based_sampler = KLBasedSampler(
                quantized_indices_memory=quantized_indices_memory,
                dataset=train_dataset,
                ratio=cfg.selection.ratio,
                device=DEVICE,
            )
            train_subset = kl_based_sampler.sample()
            logger.info(f"Train subset size: {len(train_subset) / len(train_dataset) * 100:.2f}%")
        ...
        for epoch in range(NUM_EPOCH):
            ...
            for i, (bidx, bx, bx_len, by, by_len, texts, phonetic_details, _) in enumerate(train_dataloader):
                bx = bx.to(DEVICE)
            ...
            test_cnt = 0
            with torch.no_grad():
                for i, (bidx, bx, bx_len, by, by_len, texts, phonetic_details, _) in enumerate(test_dataloader):
                    bx = bx.to(DEVICE)
                    bx_len = bx_len.to(DEVICE)
                    by = by.to(DEVICE)
                    by_len = by_len.to(DEVICE)
